...
class TelegramForwarder:
    ...
    async def forward_messages_to_channel(self, source_chat_id, destination_channel_id, keywords):
        ...
        last_message_id_1 = (await self.client.get_messages(1982197765, limit=1))[0].id

        try :
         last_message_id_2 = (await self.client.get_messages(7606609952, limit=1))[0].id 
        except :
         pass
        ...
        while True:
            ...
            messages_1 = await self.client.get_messages(1982197765, min_id=last_message_id_1, limit=None)

            try :
              messages_2 = await self.client.get_messages(7606609952, min_id=last_message_id_2, limit=None) 
              print(messages_2 +"messssss")
            except :
              pass
            


            for message in reversed(messages_1):
                ...
                if keywords:
                    if message.text and any(keyword in message.text.lower() for keyword in keywords):
                        print(f"Message 1 contains a keyword: {message.text}")
                ...
                last_message_id_1 = max(last_message_id_1, message.id)


            try :
                for message in reversed(messages_2):
                    # Check if the message text includes any of the keywords
                    if keywords:
                        if message.text and any(keyword in message.text.lower() for keyword in keywords):
                            print(f"Message 2 contains a keyword: {message.text}")
                    ...
                    last_message_id_2 = max(last_message_id_2, message.id)
